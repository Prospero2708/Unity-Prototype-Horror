...
----------------
* **`movement.cs`** — Physics-based 3D character controller. Seamlessly consumes vector events from `InputRead` to manage `Rigidbody` states via `linearVelocity`. Features a built-in **sprint scaling system** that dynamically increases player speed and camera-bobbing animation frequency based on button-hold duration.
----------------
* **`CameraLook.cs`** — A optimized First-person mouse look controller. It features independent axis rotation (yaw applied to the player root, pitch applied to the camera) and configurable vertical clamping. Most importantly, it utilizes a **frame-rate independent exponential smoothing function** (`1f - Mathf.Exp(-factor * Time.deltaTime)`) for fluid interpolation, completely eliminating camera jitter regardless of target hardware performance.
----------------
...
----------------
* **`CalculateDistance.cs`** — An optimized utility script that calculates distances between game objects to feed AI logic and distance-based triggers.
----------------
